...
export const getAllNews = createAsyncThunk("news", async (topic: string) => {
  const response = await fetch(
    `https://newsapi.org/v2/everything?q=${topic}&from=2023-05-31&to=${currentDate}&sortBy=popularity&apiKey=f34d9ee92c7e4664a0047162b8766c84`
  );
...
export const getAllNewsSources = createAsyncThunk(
  ...
  async (sourceData: string) => {
    const response = await fetch(
      sourceData
        ? `https://newsapi.org/v2/everything?sources=${sourceData}&apiKey=f34d9ee92c7e4664a0047162b8766c84`
        : `https://newsapi.org/v2/top-headlines/sources?&apiKey=f34d9ee92c7e4664a0047162b8766c84`
    );
    if (sourceData) {
